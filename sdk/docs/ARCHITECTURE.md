...
├── llm-client.ts          LLMClient — sole source of truth for all LLM calls
├── contracts.ts           AgentManifest, AgentCapability, AgentEval interfaces
├── runtime.ts             AgentRuntime — wraps a manifest, owns agent lifecycle
├── service.ts             AgentService — multi-agent dispatch and composition
...
All LLM calls flow through `LLMClient` — nowhere else in the sdk or its consumers should directly instantiate `Anthropic`. This enforces a single place for prompt caching, model pinning, and retry logic.
