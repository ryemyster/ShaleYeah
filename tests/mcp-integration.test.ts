...
2024-01-04,77.15,3.42,1195,500000,750000`;
  ...
  private async testGeowizMCP(): Promise<void> {
    ...
    const gisResult = await this.simulateMCPToolCall(geowiz, 'parse_gis_file', {
    ...
    const lasResult = await this.simulateMCPToolCall(geowiz, 'parse_well_log', {
    ...
    const detectionResult = await this.simulateMCPToolCall(geowiz, 'detect_file_format', {
      filePath: lasFilePath,
      expectedFormat: 'las'
    });
  ...
  private async testCurveSmithMCP(): Promise<void> {
    ...
    const processingResult = await this.simulateMCPToolCall(curveSmith, 'process_las_file', {
      filePath: lasFilePath,
    ...
    const statsResult = await this.simulateMCPToolCall(curveSmith, 'analyze_curve_statistics', {
      filePath: lasFilePath,
      curveName: 'GR',
  ...
  private async testEconobotMCP(): Promise<void> {
    ...
    const dataProcessingResult = await this.simulateMCPToolCall(econobot, 'process_economic_data', {
