...
await test("callLLM: reaches Anthropic SDK — auth error proves messages.create was called", async () => {
	...
	try {
		...
		try { await callLLM({ prompt: "Analyze regulatory risk for a drilling project in Texas.", maxTokens: 200 }); }
		catch (err: unknown) { caughtError = err instanceof Error ? err : new Error(String(err)); }
		assert.ok(caughtError !== null, "Expected callLLM to throw");
		assert.ok(!caughtError.message.includes("environment variable is not set"), `Guard fired instead of SDK: ${caughtError.message}`);
	} finally { if (savedKey !== undefined) { process.env.ANTHROPIC_API_KEY = savedKey; } else { delete process.env.ANTHROPIC_API_KEY; } }
});
...
await test("callLLM: throws descriptive error when ANTHROPIC_API_KEY is absent", async () => {
	...
	try {
		...
		let threw = false; let errorMessage = "";
		try { await callLLM({ prompt: "test" }); } catch (err: unknown) { threw = true; errorMessage = err instanceof Error ? err.message : String(err); }
		assert.ok(threw); assert.ok(errorMessage.includes("ANTHROPIC_API_KEY") || errorMessage.includes("API key"));
	} finally { if (savedKey !== undefined) { process.env.ANTHROPIC_API_KEY = savedKey; } }
});
