...
export function fitExponentialDecline(data: ProductionData[], product: 'oil' | 'gas'): CurveFitResult {
  ...
  const yMean = sumY / n;
  const ss_tot = y.reduce((sum, yi) => sum + Math.pow(yi - yMean, 2), 0);
  const ss_res = y.reduce((sum, yi, i) => sum + Math.pow(yi - yPred[i], 2), 0);
  ...
  for (let t = 0; t <= timeHorizon * 12; t++) { // monthly
    const years = t / 12;
...
export function fitHyperbolicDecline(data: ProductionData[], product: 'oil' | 'gas'): CurveFitResult {
  ...
  for (let Di = 0.05; Di <= 2.0; Di += 0.05) {
    const predicted = t.map(ti => qi / Math.pow(1 + b * Di * ti, 1/b));
  ...
  const predicted = t.map(ti => qi / Math.pow(1 + b * bestDi * ti, 1/b));
  const rmse = Math.sqrt(q.reduce((sum, qi, i) => sum + Math.pow(qi - predicted[i], 2), 0) / q.length);

  // Calculate EUR using Arps formula
  const timeHorizon = 30; // years
  const eur = qi / ((1-b) * bestDi) * (1 - Math.pow(1 + b * bestDi * timeHorizon, (1-b)/b));

  // Generate forecast
  const forecast: ProductionData[] = [];
  for (let t = 0; t <= timeHorizon * 12; t++) { // monthly
    const years = t / 12;
    const rate = qi / Math.pow(1 + b * bestDi * years, 1/b);
    forecast.push({
      date: `${2024 + Math.floor(years)}-${String(Math.floor((years % 1) * 12) + 1).padStart(2, '0')}-01`,
      days: years * 365,
      oil: product === 'oil' ? rate : 0,
...
function calculateR2(observed: number[], predicted: number[]): number {
  ...
  const ss_tot = observed.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0);
  const ss_res = observed.reduce((sum, val, i) => sum + Math.pow(val - predicted[i], 2), 0);
