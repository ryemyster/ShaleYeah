...
const legalTemplate: ServerTemplate = {
  ...
  tools: [
    ServerFactory.createAnalysisTool(
      ...
      async (args) => {
        const riskLevel = args.projectType === 'exploration' ? 'High' :
                         args.projectType === 'development' ? 'Medium' : 'Low';
    ...
    ServerFactory.createAnalysisTool(
      ...
      async (args) => {
        const analysis = {
          ...
          confidence: ServerUtils.calculateConfidence(0.85, 0.90)
        };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const server = new (LegalServer as any)();
  runMCPServer(server);
}