...
function evaluateInvestmentOpportunity(args: any): InvestmentDecision {
...
function developBidStrategy(args: any): BidStrategy {
...
function assessPortfolioFit(args: any): PortfolioFit {
...
function generateBidReasoning(valuation: any, strategy: string, multiplier: number): string {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const server = new (DecisionServer as any)();
  runMCPServer(server);
}