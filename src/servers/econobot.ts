...
async function performEconomicAnalysis(args: any): Promise<EconomicAnalysis> {
  try {
...
function calculateDCFMetrics(pricing: any[], costs: any[], production: any[], discountRate: number): {npv: number, irr: number, payback: number} {
  ...
  for (let year = 1; year <= years; year++) {
    const yearlyProduction = avgProduction * Math.pow(0.85, year - 1);
