  ...
  return {
    ...
    results: {
      npv: {
        ...
        stdDev: Math.sqrt(npvResults.reduce((sum, val) => sum + Math.pow(val - npvResults.reduce((a, b) => a + b) / npvResults.length, 2), 0) / npvResults.length)
      },
      irr: {
        ...
        stdDev: Math.sqrt(irrResults.reduce((sum, val) => sum + Math.pow(val - irrResults.reduce((a, b) => a + b) / irrResults.length, 2), 0) / irrResults.length)
      },
      probability: {
...
function assessRegulatoryRisk(regData: any): number {
  return Math.random() * 0.3 + 0.1;
}

function identifyKeyRisks(riskCategories: any, projectData: any): Array<any> {
...
function generateMitigationStrategies(keyRisks: any[]): string[] {
  return [
