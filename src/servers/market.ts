	...
	tools: [
		ServerFactory.createAnalysisTool(
			...
			z.object({
				...
				region: z.string(),
				timeframe: z
					.enum(["current", "1year", "5year", "10year"])
					.default("1year"),
						...
						oil:
							args.commodity !== "gas"
								? {
										...
										volatility:
											Math.round((Math.random() * 0.3 + 0.1) * 100) / 100,
									}
								: undefined,
						gas:
							args.commodity !== "oil"
								? {
										...
										trend: Math.random() > 0.5 ? "bullish" : "bearish",
										volatility:
											Math.round((Math.random() * 0.4 + 0.15) * 100) / 100,
									}
								: undefined,
					},
					forecast: {
						shortTerm:
							"6-month outlook remains volatile due to geopolitical factors",
		...
		ServerFactory.createAnalysisTool(
			...
			z.object({
				...
				metrics: z
					.array(z.string())
					.default(["market_share", "production", "costs"]),
				outputPath: z.string().optional(),
			}),
			async (args) => {
				const analysis = {
					...
					competitors: args.competitors.map((comp: string) => ({
						...
						strategy: ["growth", "optimization", "consolidation"][
							Math.floor(Math.random() * 3)
						],
						strengths: ["operational efficiency", "technology", "portfolio"][
							Math.floor(Math.random() * 3)
						],
					})),
					landscape: {
