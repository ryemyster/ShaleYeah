...
const drillingTemplate: ServerTemplate = {
  ...
  tools: [
    ServerFactory.createAnalysisTool(
      ...
      async (args) => {
        const wellCost = args.wellParameters.targetDepth * (
          args.wellParameters.wellType === 'horizontal' ? 180 :
          args.wellParameters.wellType === 'directional' ? 140 : 120
        );
        ...
        const analysis = {
          ...
          drilling: {
            estimatedDays: Math.ceil(args.wellParameters.targetDepth /
            ...
            casingProgram: [
              ...
              args.wellParameters.wellType === 'horizontal' ?
                '9 5/8" intermediate to TD' : '7" production to TD'
            ],
            completion: args.wellParameters.wellType === 'horizontal' ?
              'Multi-stage fracturing' : 'Conventional completion'
          },
          costs: {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const server = new (DrillingServer as any)();
  runMCPServer(server);
}