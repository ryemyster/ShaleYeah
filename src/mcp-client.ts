...
export class ShaleYeahMCPClient {
	...
	async executeAnalysis(request: AnalysisRequest, session?: Session): Promise<WorkflowResult> {
		...
		// Guard: default to 0 when no analyses succeeded so we don't divide by zero
		...
		return {
			...
			// 4 = minimum viable quorum: geo + econ + risk + decision. Fewer than that and the recommendation is unsound.
			success: successfulResults.length >= 4,
		};
	}
