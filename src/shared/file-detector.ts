...
export class FileFormatDetector {
	private static readonly FORMAT_SIGNATURES: Record<
		string,
		{
			extensions: string[];
	...
	async extractMetadata(filePath: string): Promise<Record<string, any>> {
		...
		switch (detected.format) {
			case "las":
				return {
					...baseMetadata,
					...(await this.extractLASMetadata(filePath)),
				};
			case "shapefile":
				return {
					...baseMetadata,
					...(await this.extractShapefileMetadata(filePath)),
				};
			case "geojson":
				return {
					...baseMetadata,
					...(await this.extractGeoJSONMetadata(filePath)),
				};
			case "excel":
				return {
					...baseMetadata,
					...(await this.extractExcelMetadata(filePath)),
				};
			default:
				return baseMetadata;
		}
	...
	private async identifyFormat(
		...
		for (const [formatName, signature] of Object.entries(FileFormatDetector.FORMAT_SIGNATURES)) {
			...
			if (signature.extensions.includes(ext)) {
				...
				if (signature.headerPattern?.test(content)) {
					return { name: formatName, isValid: true, errors: [] };
				}

				// Custom validator
				if (signature.validator?.(buffer)) {
					return { name: formatName, isValid: true, errors: [] };
				}
