...
export class ExcelParser {
  ...
  async parseExcelFile(filePath: string): Promise<ExcelData> {
    ...
    try {
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        const sheetData = this.parseWorksheet(worksheet, sheetName);
  ...
  async parseCSVFile(filePath: string, options: {
    ...
    try {
      const content = await fs.readFile(filePath, { encoding: options.encoding as BufferEncoding || 'utf-8' });
  ...
  private parseWorksheet(worksheet: XLSX.WorkSheet, sheetName: string): ExcelSheet {
    ...
    const formulaCells = this.countFormulaCells(worksheet);
  ...
  private detectExcelVersion(filePath: string): string {
    ...
    switch (ext) {
      ...
      case 'xlsm': return 'Excel 2007+ (Macro-enabled)';
  ...
  private countFormulaCells(worksheet: XLSX.WorkSheet): number {
    let count = 0;
    const range = XLSX.utils.decode_range(worksheet['!ref'] || 'A1:A1');
