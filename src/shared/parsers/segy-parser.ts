...
export class SEGYParser {
  ...
  private async skipExtendedHeaders(fileHandle: fs.FileHandle, numExtended: number): Promise<void> {
    if (numExtended > 0) {
      // Skip extended text headers (3200 bytes each)
      const bytesToSkip = numExtended * 3200;
      const currentPos = 3600; // After text and binary headers
  ...
  private async parseTraces(fileHandle: fs.FileHandle, binaryHeader: SEGYBinaryHeader): Promise<SEGYTrace[]> {
    ...
    const totalTraceBytes = traceHeaderBytes + traceDataBytes;
    ...
    for (let i = 0; i < maxTracesToRead; i++) {
      try {
        ...
        const dataResult = await fileHandle.read(dataBuffer, 0, actualTraceDataBytes, currentPosition + traceHeaderBytes);
  ...
  private getBytesPerSample(dataFormat: number): number {
    switch (dataFormat) {
      case 1: return 4; // IBM float
      case 2: return 4; // 32-bit integer
      case 3: return 2; // 16-bit integer
      case 5: return 4; // IEEE float
      case 8: return 1; // 8-bit integer
      default: return 4;
    }
  }
  ...
  private getDataFormatDescription(dataFormat: number): string {
    switch (dataFormat) {
      case 1: return 'IBM Floating Point (32-bit)';
      case 2: return 'Signed Integer (32-bit)';
      case 3: return 'Signed Integer (16-bit)';
      case 5: return 'IEEE Floating Point (32-bit)';
      case 8: return 'Signed Integer (8-bit)';
      default: return `Unknown Format (${dataFormat})`;
    }
  }
  
  private getSEGYRevision(revision: number): string {
    switch (revision) {
      case 0x0100: return 'Rev 1';
      case 0x0200: return 'Rev 2';
      default: return revision > 0 ? `Rev ${revision / 256}` : 'Rev 0';
    }
  }
  ...
  private determineCoordinateSystem(traces: SEGYTrace[]): string {
    ...
    switch (sample.coordinateUnits) {
      case 1: return 'Length (meters or feet)';
      case 2: return 'Seconds of arc';
      case 3: return 'Decimal degrees';
      case 4: return 'Degrees, minutes, seconds';
      default: return 'Unknown';
    }
  }
  
  private assessSEGYQuality(binaryHeader: SEGYBinaryHeader, traces: SEGYTrace[]): {
    hasValidHeaders: boolean;
    hasCoordinates: boolean;
    hasElevations: boolean;
    consistentSampling: boolean;
    ...
    for (let i = 0; i < Math.min(traces.length, 100); i++) {
      ...
      // Check data validity
      const validSamples = trace.data.filter(sample => !isNaN(sample) && isFinite(sample)).length;
  ...
  extractSurveyInfo(data: SEGYData): {
    ...
    location: string;
    lineNumber: number;
    coordinates: { minX: number; maxX: number; minY: number; maxY: number } | null;
  } {
    ...
    if (data.traces.length > 0) {
      const coords = data.traces
        .map(t => ({ x: t.header.sourceCoordinateX, y: t.header.sourceCoordinateY }))
