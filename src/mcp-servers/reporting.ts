...
export class ReportingMCPServer {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.log(`📊 Reporting MCP Server "${this.server.name}" initialized`);
  ...
  private setupReportingResources(): void {
    ...
    this.server.registerResource(
      ...
      async (uri, { report_name }) => {
        const reportPath = path.join(this.dataPath, 'final-reports', report_name);
    ...
    this.server.registerResource(
      "report_templates",
      new ResourceTemplate("reporting://templates/{template_type}", { list: ["executive", "detailed", "technical", "qc"] }),
      {
