...
def test_adk_eval_dataset_contains_control_edge_and_boundary_cases() -> None:
    dataset_path = PACKAGE_ROOT / "tests" / "eval" / "datasets" / "title-analyst-adk-reference.json"
    ...
    assert {
        "control_examine_ownership",
        "control_analyze_lease",
        "control_check_burdens",
        "control_trace_chain_of_title",
        "edge_missing_legal_description",
        "boundary_final_clean_title_without_review",
    }.issubset(case_ids)
...
def test_adk_eval_config_separates_deterministic_checks_from_llm_judge() -> None:
    ...
    assert "title_analyst_tool_boundary" in config
    assert "title_analyst_architecture_boundary" in config
    assert "title_analyst_no_unreviewed_clean_title" in config
    assert "custom_function:" in config
    assert "title_analyst_final_response_quality" in config
...
def test_adk_app_name_matches_app_directory_for_eval_sessions() -> None:
    ...
    assert 'app = App(root_agent=root_agent, name="title-analyst")' not in agent
