...
def test_adk_manifest_exists_inside_title_analyst_package() -> None:
    manifest = (PACKAGE_ROOT / "agents-cli-manifest.yaml").read_text()

    assert 'name: "title-analyst"' in manifest
    assert 'agent_directory: "app"' in manifest
    assert 'deployment_target: "none"' in manifest


def test_adk_python_entrypoint_exists_inside_title_analyst_package() -> None:
...
def test_title_analyst_agent_has_no_dangling_npm_surface() -> None:
