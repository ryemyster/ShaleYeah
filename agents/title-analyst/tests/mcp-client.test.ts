...
async function runTests(): Promise<void> {
	...
	await test("callTitleTool is exported as a function", () => {
		assert.strictEqual(
			typeof callTitleTool,
			"function",
			"callTitleTool must be exported",
		);
	});
	...
	await test('titleAnalystConfig.mcpServers["title"].url is http://localhost:3010', () => {
		...
		assert.strictEqual(
			conn.url,
			"http://localhost:3010",
			"title URL must be localhost:3010",
		);
	});
	...
	await test("all 4 title-analyst tools declare mcpServer: 'title'", () => {
		const wrong = titleAnalystManifest.tools.filter(
			(t) => t.mcpServer !== "title",
		);
		assert.strictEqual(
			wrong.length,
			0,
			`Tools without mcpServer='title': ${wrong.map((t) => t.name).join(", ")}`,
		);
	});
	...
	await test("runTitleAnalystTask is exported as a function", () => {
		assert.strictEqual(
			typeof runTitleAnalystTask,
			"function",
			"runTitleAnalystTask must be exported",
		);
	});
	...
	await test("runTitleAnalystTask throws when no ANTHROPIC_API_KEY is set", async () => {
		...
		let threw = false;
		try {
			await runTitleAnalystTask(
				"examine title for Section 12 Reeves County TX",
			);
		} catch (err) {
			threw = true;
			const msg = err instanceof Error ? err.message : String(err);
	...
	await test("runTitleAnalystTask hits callLLM when API key is present (auth error confirms path)", async () => {
		...
		assert.ok(
			threw,
			"runTitleAnalystTask with invalid key must throw (proves callLLM was hit)",
		);
	});
	...
	await test("runTitleAnalystTask accepts runtime option (signature check)", () => {
		...
		assert.ok(
			params >= 1,
			"runTitleAnalystTask must accept at least a goal argument",
		);
	});
	...
	if (live) {
		await test("[integration] callTitleTool routes examine_ownership through title MCP", async () => {
			const result = await callTitleTool(
				titleAnalystConfig.mcpServers?.title.url,
				"examine_ownership",
				{
					propertyDescription: "Section 12 Block A",
					county: "Reeves",
	...
	if (live && HAS_API_KEY) {
		await test("[integration] runTitleAnalystTask completes a multi-step title task", async () => {
			...
			assert.strictEqual(
				typeof answer,
				"string",
				"runTitleAnalystTask must return a string",
			);
