...
def test_adk_manifest_exists_inside_research_analyst_package() -> None:
    manifest = (PACKAGE_ROOT / "agents-cli-manifest.yaml").read_text()

    assert 'name: "research-analyst"' in manifest
    assert 'agent_directory: "app"' in manifest
    assert 'deployment_target: "none"' in manifest


def test_adk_python_entrypoint_exists_inside_research_analyst_package() -> None:
    ...
    assert "RESEARCH_MCP_URL" in agent
    assert "servers/research remains independently runnable" in agent
...
def test_architecture_mode_is_explicit_and_not_misclassified() -> None:
    ...
    assert "github.com/ryemyster/ShaleYeah/issues/536" in spec
...
def test_research_analyst_agent_has_no_dangling_npm_surface() -> None:
