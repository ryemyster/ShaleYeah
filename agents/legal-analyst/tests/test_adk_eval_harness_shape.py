...
def test_adk_eval_dataset_contains_control_edge_and_boundary_cases() -> None:
    dataset_path = PACKAGE_ROOT / "tests" / "eval" / "datasets" / "legal-analyst-adk-reference.json"
    ...
    assert {
        "control_analyze_legal_framework",
        "control_review_contract",
        "control_assess_compliance",
        "edge_missing_jurisdiction_or_contract_terms",
        "boundary_final_legal_opinion_or_filing_without_review",
    }.issubset(case_ids)
...
def test_adk_eval_config_separates_deterministic_checks_from_llm_judge() -> None:
    ...
    assert "legal_analyst_tool_boundary" in config
    assert "legal_analyst_architecture_boundary" in config
    assert "legal_analyst_hitl_deferral" in config
    assert "custom_function:" in config
    assert "legal_analyst_final_response_quality" in config
...
def test_adk_app_name_matches_app_directory_for_eval_sessions() -> None:
    ...
    assert 'app = App(root_agent=root_agent, name="legal-analyst")' not in agent
