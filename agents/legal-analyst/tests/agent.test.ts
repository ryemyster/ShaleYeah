...
console.log("📋 Testing manifest and config validation...");
{
	...
	assert(
		legalAnalystManifest.id === "legal-analyst",
		"Agent id is legal-analyst",
	);
	assert(
		legalAnalystManifest.persona.name === "Legatus Juridicus",
		"Persona is Legatus Juridicus",
	);
	assert(
		legalAnalystManifest.tools.length === 3,
		"Legal analyst exposes 3 legal tools",
	);
	assert(
		legalAnalystManifest.tools.every((t) =>
			t.name.startsWith("legal-analyst."),
		),
		"All tools follow legal-analyst. naming convention",
	);
	assert(
		legalAnalystManifest.tools.every(
	...
	const allToolScopes = new Set(
		legalAnalystManifest.tools.flatMap((t) => t.requiredScopes),
	);
	assert(
		[...allToolScopes].every((s) =>
			legalAnalystManifest.requiredScopes.includes(s),
		),
		"Manifest requiredScopes is a superset of all tool requiredScopes",
	);
	const llmReq = legalAnalystManifest.providerRequirements.find(
		(p) => p.type === "llm",
	);
	assert(
...
console.log("\n🔎 Testing progressive discovery...");
{
	...
	const schema = runtime.discover(
		"schema",
		"legal-analyst.analyze_legal_framework",
	);
	assert(
		schema?.inputSchema !== undefined,
		"Schema discovery returns analyze_legal_framework input schema",
	);
	assert(
...
console.log("\n🧭 Testing organization-owned model routing...");
{
	...
	const analysisTool = legalAnalystManifest.tools.find(
		(t) => t.name === "legal-analyst.analyze_legal_framework",
	);
	assert(
		analysisTool !== undefined,
		"analyze_legal_framework tool is declared in manifest",
	);
	...
	assert(
		overrideRoute.provider === "acme-legal-llm",
...
);
{
	...
	const modelRequirements = new Set(
		legalAnalystManifest.tools.map((t) => t.modelRequirement),
	);
	assert(
	...
	assert(
		typeof standardBinding?.model === "string" &&
			standardBinding.model.length > 0,
...
{
	const approvalRequired = legalAnalystManifest.tools.filter(
		(t) => t.requiresHumanApproval,
	);
	assert(
		approvalRequired.length === 0,
		"No legal-analyst tool requires human approval by default",
	);
	...
	if (blocked.status === "approval_required") {
		...
		assert(
			blocked.challenge.agentId === "legal-analyst",
			"Challenge identifies legal-analyst agent",
		);
	}
...
console.log("\n🧪 Testing evals policy is configured correctly...");
{
	...
	assert(
		legalAnalystConfig.evals.checks.schema === "blocking",
		"Schema eval is blocking",
	);
	assert(
		legalAnalystConfig.evals.checks.redactSecrets === "blocking",
	...
	assert(
		...
		"All legal-analyst tools declare eval profiles",
	);
	assert(
...
console.log("\n🏥 Testing health endpoint and standalone boot...");
{
	...
	assert(
		health.agentId === "legal-analyst",
		"Health endpoint identifies legal-analyst",
	);
	assert(
		health.status !== "not_ready",
		"Legal analyst boots without external dependencies",
	);

	const manifest = await endpoint.manifest();
	assert(
		manifest.id === "legal-analyst",
		"Endpoint exposes legal-analyst manifest",
	);
...
try {
	new LocalAgentRuntime({
		...
		handlers: Object.fromEntries(
			legalAnalystManifest.tools.map((t) => [t.name, async () => ({})]),
		),
	});
...
console.log("\n══════════════════════════════════════════════");
console.log(
	`Legal Analyst Agent Contract Tests: ${passed} passed, ${failed} failed`,
);
console.log("══════════════════════════════════════════════");
