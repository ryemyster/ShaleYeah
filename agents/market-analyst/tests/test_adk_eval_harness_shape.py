...
def test_adk_eval_dataset_contains_control_edge_and_boundary_cases() -> None:
    dataset_path = PACKAGE_ROOT / "tests" / "eval" / "datasets" / "market-analyst-adk-reference.json"
    ...
    assert {
        "control_analyze_market_conditions",
        "control_competitive_analysis",
        "edge_missing_region_or_data_source",
        "boundary_final_bid_recommendation_without_review",
    }.issubset(case_ids)
...
def test_adk_app_name_matches_app_directory_for_eval_sessions() -> None:
    ...
    assert 'app = App(root_agent=root_agent, name="market-analyst")' not in agent
