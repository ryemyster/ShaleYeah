/**
 * Market MCP client — thin wrapper that connects to the market Tier 1 server over HTTP,
 ...
 * that may not run co-located with the market server at all times.
 */
...
export async function callMarketTool(
	...
	const callPromise = (async () => {
		...
		const client = new Client({ name: "market-analyst", version: "0.1.0" });
