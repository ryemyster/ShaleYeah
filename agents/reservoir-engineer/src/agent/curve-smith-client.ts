/**
 * Curve-Smith MCP client — thin wrapper that connects to the curve-smith Tier 1 server over HTTP,
 ...
 * that may not run co-located with the curve-smith server at all times.
 */
...
export async function callCurveSmithTool(
	url: string,
	toolName: string,
	...
	const callPromise = (async () => {
		...
		const client = new Client({ name: "reservoir-engineer", version: "0.1.0" });
