/**
 * Decision MCP client — thin wrapper that connects to the decision Tier 1 server over HTTP,
 ...
 * that may not run co-located with the decision server at all times.
 */
...
export async function callDecisionTool(
	...
	const callPromise = (async () => {
		...
		const client = new Client({ name: "investment-chair", version: "0.1.0" });
