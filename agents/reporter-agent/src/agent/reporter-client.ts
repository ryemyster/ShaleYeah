/**
 * Reporter MCP client — thin wrapper that connects to the reporter Tier 1 server over HTTP,
 ...
 * that may not run co-located with the reporter server at all times.
 */
...
export async function callReporterTool(
	url: string,
	toolName: string,
	...
	const callPromise = (async () => {
		...
		const client = new Client({ name: "reporter-agent", version: "0.1.0" });
