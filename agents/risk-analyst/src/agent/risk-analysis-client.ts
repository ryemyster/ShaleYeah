/**
 * Risk-Analysis MCP client — thin wrapper that connects to the risk-analysis Tier 1 server over HTTP,
 ...
 * that may not run co-located with the risk-analysis server at all times.
 */
...
export async function callRiskAnalysisTool(
	url: string,
	toolName: string,
	...
	const callPromise = (async () => {
		...
		const client = new Client({ name: "risk-analyst", version: "0.1.0" });
