...
console.log("📋 Testing manifest and config validation...");
{
	...
	assert(
		drillingEngineerManifest.tools.length === 3,
		"Drilling engineer exposes 3 drilling tools",
	);
	assert(
		drillingEngineerManifest.tools.every((t) =>
			t.name.startsWith("drilling-engineer."),
		),
		"All tools follow drilling-engineer. naming convention",
	);
	assert(
		drillingEngineerManifest.tools.every(
	...
	const allToolScopes = new Set(
		drillingEngineerManifest.tools.flatMap((t) => t.requiredScopes),
	);
	assert(
		[...allToolScopes].every((s) =>
			drillingEngineerManifest.requiredScopes.includes(s),
		),
		"Manifest requiredScopes is a superset of all tool requiredScopes",
	);
	const llmReq = drillingEngineerManifest.providerRequirements.find(
		(p) => p.type === "llm",
	);
	assert(
...
console.log("\n🔎 Testing progressive discovery...");
{
	...
	assert(
		...
		"Schema discovery returns design_drilling_program input schema",
	);
	assert(
...
console.log("\n🧭 Testing organization-owned model routing...");
{
	...
	if (!live) {
		console.log(
			"  ⚠️  [skipped] drilling server not reachable — execute() tests require live server",
		);
...
);
{
	...
	const modelRequirements = new Set(
		drillingEngineerManifest.tools.map((t) => t.modelRequirement),
	);
	assert(
...
console.log("\n🙋 Testing HITL policy wires through to config...");
{
	...
	if (live) {
		...
		assert(
			...
			"Drilling tools complete without approval challenge",
		);
	...
	if (blocked.status === "approval_required") {
		...
		assert(
			blocked.challenge.agentId === "drilling-engineer",
			"Challenge identifies drilling-engineer agent",
		);
	}
...
console.log("\n🧪 Testing evals policy is configured correctly...");
{
	...
	if (!live) {
		...
		assert(
			result.status === "completed",
			"design_drilling_program completes for eval test",
		);
...
console.log("\n🏥 Testing health endpoint and standalone boot...");
{
	...
	assert(
		health.agentId === "drilling-engineer",
		"Health endpoint identifies drilling-engineer",
	);
	assert(
		health.status !== "not_ready",
		"Drilling engineer boots without external dependencies",
	);

	const manifest = await endpoint.manifest();
	assert(
		manifest.id === "drilling-engineer",
		"Endpoint exposes drilling-engineer manifest",
	);
	assert(manifest.tools.length === 3, "Endpoint manifest has 3 tools");

	const toolSchema = await endpoint.discoveryToolSchema(
		"drilling-engineer.estimate_well_costs",
	);
...
console.log("\n🔀 Testing model routing resolution...");
{
	const standardAnalysis =
		drillingEngineerConfig.modelRouting["standard-analysis"];
	assert(
		standardAnalysis !== undefined,
		"standard-analysis binding is present",
	);
	assert(
...
console.log("\n══════════════════════════════════════════════");
console.log(
	`Drilling Engineer Agent Contract Tests: ${passed} passed, ${failed} failed`,
);
console.log("══════════════════════════════════════════════");
