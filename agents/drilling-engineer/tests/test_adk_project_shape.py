...
def test_adk_manifest_exists_inside_drilling_engineer_package() -> None:
    manifest = (PACKAGE_ROOT / "agents-cli-manifest.yaml").read_text()

    assert 'name: "drilling-engineer"' in manifest
    assert 'agent_directory: "app"' in manifest
    assert 'deployment_target: "none"' in manifest


def test_adk_python_entrypoint_exists_inside_drilling_engineer_package() -> None:
    ...
    assert "DRILLING_MCP_URL" in agent
    assert "servers/drilling remains independently runnable" in agent
...
def test_drilling_engineer_agent_has_no_dangling_npm_surface() -> None:
