...
async function runTests(): Promise<void> {
	...
	await test("all 3 drilling-engineer tools declare mcpServer: 'drilling'", () => {
		...
		assert.strictEqual(wrong.length, 0, `Tools without mcpServer='drilling': ${wrong.map((t) => t.name).join(", ")}`);
	});
	...
	await test("runDrillingEngineerTask is exported as a function", () => {
		assert.strictEqual(typeof runDrillingEngineerTask, "function", "runDrillingEngineerTask must be exported");
	});
