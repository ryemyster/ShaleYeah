...
def test_adk_manifest_exists_inside_development_planner_package() -> None:
    manifest = (PACKAGE_ROOT / "agents-cli-manifest.yaml").read_text()

    assert 'name: "development-planner"' in manifest
    assert 'agent_directory: "app"' in manifest
    assert 'deployment_target: "none"' in manifest


def test_adk_python_entrypoint_exists_inside_development_planner_package() -> None:
    ...
    assert "DEVELOPMENT_MCP_URL" in agent
    assert "servers/development remains independently runnable" in agent
...
def test_architecture_mode_is_explicit_and_not_misclassified() -> None:
    ...
    assert "docs/development-planner-role.md" in spec
...
def test_development_planner_agent_has_no_dangling_npm_surface() -> None:
