...
console.log("📋 Testing manifest and config validation...");
{
	...
	assert(
		developmentPlannerManifest.id === "development-planner",
		"Agent id is development-planner",
	);
	assert(
	...
	assert(
		developmentPlannerManifest.tools.length === 3,
		"Development planner exposes 3 development tools",
	);
	assert(
		developmentPlannerManifest.tools.every((t) =>
			t.name.startsWith("development-planner."),
		),
		"All tools follow development-planner. naming convention",
	);
	assert(
		developmentPlannerManifest.tools.every(
	...
	const allToolScopes = new Set(
		developmentPlannerManifest.tools.flatMap((t) => t.requiredScopes),
	);
	assert(
		[...allToolScopes].every((s) =>
			developmentPlannerManifest.requiredScopes.includes(s),
		),
		"Manifest requiredScopes is a superset of all tool requiredScopes",
	);
	const llmReq = developmentPlannerManifest.providerRequirements.find(
		(p) => p.type === "llm",
	);
	assert(
...
console.log("\n🔎 Testing progressive discovery...");
{
	...
	assert(
		...
		"Schema discovery returns create_development_plan input schema",
	);
	assert(
...
console.log("\n🧭 Testing organization-owned model routing...");
{
	...
	assert(
		planTool !== undefined,
		"create_development_plan tool is declared in manifest",
	);

	const deterministicRoute =
		developmentPlannerConfig.modelRouting.deterministic;
	assert(deterministicRoute !== undefined, "Deterministic route is configured");
	assert(
		deterministicRoute.provider === "rule-based",
		"Deterministic tools use rule-based provider",
	);

	const overrideRoute = customRouting["standard-analysis"];
	assert(
		overrideRoute.provider === "acme-development-llm",
		"Operator override populates provider",
	);
	assert(
		overrideRoute.model === "operator-selected-model",
		"Operator override populates model",
	);
}
...
);
{
	...
	const modelRequirements = new Set(
		developmentPlannerManifest.tools.map((t) => t.modelRequirement),
	);
	assert(
	...
	assert(
		modelRequirements.has("deterministic"),
		"deterministic requirement present in tool suite",
	);
...
{
	const approvalRequired = developmentPlannerManifest.tools.filter(
		(t) => t.requiresHumanApproval,
	);
	assert(
		approvalRequired.length === 0,
		"No development-planner tool requires human approval by default",
	);
	...
	if (blocked.status === "approval_required") {
		...
		assert(
			blocked.challenge.agentId === "development-planner",
			"Challenge identifies development-planner agent",
		);
	}
...
console.log("\n🧪 Testing evals policy is configured correctly...");
{
	...
	assert(
		developmentPlannerConfig.evals.checks.schema === "blocking",
		"Schema eval is blocking",
	);
	assert(
		developmentPlannerConfig.evals.checks.redactSecrets === "blocking",
		"Secret-redaction eval is blocking",
	);

	const profileTools = developmentPlannerManifest.tools.filter(
		(t) => t.evalProfile,
	);
	assert(
		profileTools.length === 3,
		"All development-planner tools declare eval profiles",
	);
	assert(
...
console.log("\n🏥 Testing health endpoint and standalone boot...");
{
	...
	assert(
		health.agentId === "development-planner",
		"Health endpoint identifies development-planner",
	);
	assert(
		health.status !== "not_ready",
		"Development planner boots without external dependencies",
	);

	const manifest = await endpoint.manifest();
	assert(
		manifest.id === "development-planner",
		"Endpoint exposes development-planner manifest",
	);
...
try {
	new LocalAgentRuntime({
		...
		handlers: Object.fromEntries(
			developmentPlannerManifest.tools.map((t) => [t.name, async () => ({})]),
		),
	});
...
console.log("\n══════════════════════════════════════════════");
console.log(
	`Development Planner Agent Contract Tests: ${passed} passed, ${failed} failed`,
);
console.log("══════════════════════════════════════════════");
