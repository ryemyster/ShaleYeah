...
### Added
- **Package-local ADK project shape** (#587) — added `agents-cli-manifest.yaml`, `.agents-cli-spec.md`, `pyproject.toml`, `AGENTS.md`, and `app/agent.py` under `agents/geologist` only. The ADK slice makes Geowiz backend selection explicit while preserving `servers/geowiz` as the independently runnable MCP backend and keeping repo root free of ADK scaffolding.
